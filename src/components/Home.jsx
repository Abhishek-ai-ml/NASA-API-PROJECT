...
const Home = ({ data }) => {
  ...
  return (
    <div className="w-full h-screen relative">
        ...
        <div className={`absolute text-white right-5 bottom-20 bg-transparent ${knowMore ? "p-8 bg-white rounded lg text-blue-800 w-[600px] shadow-[0_3px_10px_rgb(0,0,0,0.2)]" : ""} transition-all duration-200 ease-in-out`}>{knowMore ? data.explanation : data.explanation?.substr(0, 80)}... <button className="underline" onClick={() =>setKnowMore(!knowMore)}>{knowMore ? "Show Less" : "Know More"}</button></div>
